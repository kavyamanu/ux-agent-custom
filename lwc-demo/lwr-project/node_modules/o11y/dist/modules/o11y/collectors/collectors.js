...
class ProtobufUtil {
    ...
    getSchemaType(schema) {
        ...
        if (!type) {
            const schemaInstance = __protobufjs.Root.fromJSON(schema.pbjsSchema);
    ...
    encode(schema, data) {
        const type = this.validate(schema, data);
        const message = type.fromObject(data);
        const encoded = type.encode(message).finish();
        return encoded;
    }
    encodePayload(rawPayload) {
        if ((rawPayload === null || rawPayload === void 0 ? void 0 : rawPayload.schema) && rawPayload.payload) {
            return {
    ...
    getLogMessage(schema, data, logMeta) {
        return {
    ...
    processRawLogMessage(schema, raw) {
...
const root = __protobufjs.Root.fromJSON(coreEnvelopeSchema.pbjsSchema);
const _coreEnvelopeType = root.lookupType('CoreEnvelope');
function processRawContents(contentsRaw) {
    const processed = Object.assign({}, contentsRaw, {
        messages: new Map()
    });
    contentsRaw.messages.forEach(function (rawMessages, schema) {
        processed.messages.set(schema, rawMessages.map((msg) => protobufUtil.processRawLogMessage(schema, msg)));
    });
    return processed;
}
function envelopeContents(contents) {
    return CoreEnvelopeBuilder.buildFrom(contents);
}
function encodeCoreEnvelope(coreEnvelope) {
...
function encodeCoreEnvelopeContentsRaw(contentsRaw) {
    const contents = processRawContents(contentsRaw);
    const coreEnvelope = envelopeContents(contents);
    const encoded = encodeCoreEnvelope(coreEnvelope);
    return encoded;
}

var UploadMode;
(function (UploadMode) {
    UploadMode[UploadMode["fetchBinary"] = 0] = "fetchBinary";
    UploadMode[UploadMode["fetchFile"] = 1] = "fetchFile";
...
class CoreCollector {
    constructor(uploadEndpoint, uploadMode, environment, options) {
        this._messagesLimit = defaultMessagesLimit;
        this._metricsLimit = defaultMetricsLimit;
        this._formDataKey = defaultFormDataKey;
        this._uploadInterval = defaultMaxDelayBeforeUpload;
        this._messageSize = 0;
        ...
        if (options) {
            ...
            const msglim = options.messagesLimit;
            if (msglim !== undefined) {
            ...
            const metlim = options.metricsLimit;
            if (metlim !== undefined) {
            ...
            if (utility.requireArgumentIfDefined(options.formDataKeyName, 'options.formDataKeyName', 'string')) {
    ...
    set uploadInterval(uploadInterval) {
    ...
    set uploadEndpoint(uploadEndpoint) {
        utility.requireArgumentIfDefined(uploadEndpoint, 'uploadEndpoint', 'string');
        this._uploadEndpoint = uploadEndpoint;
    }
    get uploadOptionsCallback() {
        return this._uploadOptionsCallback;
    }
    set uploadOptionsCallback(uploadOptionsCallback) {
        utility.requireArgumentIfDefined(uploadOptionsCallback, 'uploadOptionsCallback', 'function');
        this._uploadOptionsCallback = uploadOptionsCallback;
    }
    get uploadMode() {
        return this._uploadMode;
    }
    set uploadMode(uploadMode) {
    ...
    _shouldUpload() {
        return ((this._immediateUpload && this.hasData) ||
            this.messagesCount >= this._messagesLimit ||
            this.metricsCount >= this._metricsLimit);
    }
    async _upload(userInitiated) {
        this._restartTimer();
        if (!this._uploadEndpoint || this._uploadMode === UploadMode.noUpload) {
            return undefined;
        }
        const contents = this._getContentsOfCoreEnvelope(true);
        return this._uploadContents(contents, userInitiated);
    }
    async _uploadContents(contents, userInitiated) {
        let requestInit = this._getRequestInit(contents);
        const ur = {
                    ...
                    if (options.headers && requestInit.headers) {
                        if (options.headers instanceof Headers) {
                            for (const key in requestInit.headers) {
                                if (Object.prototype.hasOwnProperty.call(requestInit.headers, key)) {
                                    options.headers.set(key, requestInit.headers[key]);
                                }
                            }
                        }
    ...
    _getRequestInit(contents) {
        const coreEnvelope = envelopeContents(contents);
        const coreEnvelopeEncoded = encodeCoreEnvelope(coreEnvelope);
        let requestInit;
        switch (this._uploadMode) {
            ...
            case UploadMode.fetchFile:
                ...
                formData.append(this._formDataKey || defaultFormDataKey, new Blob([coreEnvelopeEncoded]));
                requestInit = {
                    method: 'POST',
                    body: formData
                };
    ...
    getProtoEncodedCoreEnvelope() {
        return this._buildProtoEncodedCoreEnvelope(true);
    }
    getContentsOfCoreEnvelope() {
        return this._getContentsOfCoreEnvelope(true);
    }
    _getContentsOfCoreEnvelope(extract) {
        const contents = {
            staticAttributes: this._staticAttributes,
            messages: this._messageBuffers.getAllMessages(extract)
        };
        if (this._metricsExtractors) {
            contents.upCounters = CoreEnvelopeBuilder.getUpCounters(this._metricsExtractors.getAllUpCounters(), extract);
            contents.valueRecorders = CoreEnvelopeBuilder.getValueRecorders(this._metricsExtractors.getAllValueRecorders(), extract);
            contents.bucketHistograms = CoreEnvelopeBuilder.getBucketHistograms(this._metricsExtractors.getAllBucketHistograms(), extract);
        }
        if (extract) {
            this._messageSize = 0;
        }
    ...
    _buildProtoEncodedCoreEnvelope(extract) {
        const contents = this._getContentsOfCoreEnvelope(extract);
        const coreEnvelope = envelopeContents(contents);
...
class HybridCollector {
    constructor() {
        ...
        this._observability = __nimbus.plugins.observability;
        setInterval(() => {
    ...
    collect(schema, data, logMeta) {
        ...
        const logMessage = protobufUtil.getLogMessage(schema, data, logMeta);
        const logMetaObs = {
            timestamp: logMeta.timestamp,
            rootId: logMeta.rootId,
    ...
    notifyIdleDetected(timestamp) {
        this._observability.idleDetected(timestamp);
    }
    _collectMetrics() {
        if (this._metricsExtractors) {
            const upCounters = this._metricsExtractors.getAllUpCounters();
            const valueRecorders = this._metricsExtractors.getAllValueRecorders();
            const bucketHistograms = this._metricsExtractors.getAllBucketHistograms();
            const recorders = [];
            const counters = [];
            const histograms = [];
    ...
    _getMetricTags(metric) {
        const metricsTags = metric.getTags();
        if (metricsTags) {
...
export { CoreCollector, HybridCollector, encodeCoreEnvelopeContentsRaw, processRawContents };
