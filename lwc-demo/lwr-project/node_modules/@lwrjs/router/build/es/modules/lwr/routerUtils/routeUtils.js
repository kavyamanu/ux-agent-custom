...
export function matchRouteByUrl(url, routeDefs, basePath = '', i18n = DEFAULT_I18N_ROUTER_CONFIG, options) {
    ...
    let matchInfo;
    // If there is no Route Definition match, return null
    if (!routeDef) {
        return null;
    }
    else {
...
export function getUrlFromPageReferenceAndRouteDef(pageReference, routeDef, basePath = '', i18n = DEFAULT_I18N_ROUTER_CONFIG, options) {
    ...
    // Get all the route params specified in the query string
    const queryParamNames = getQueryNames(compiledQuery);
    // For each queryParam, get the corresponding pageReference attribute or state value.
...
function getQueryObjectForParametersAndPageReference(pageReference, queryParameters, routeDef) {
    const { compiledQuery, original: { page: { state: stateBindings = {} }, }, } = routeDef;
    const { state: refState = {} } = pageReference || {};
    const nonParamState = {};
    Object.keys(refState)
        .filter((stateKey) => {
