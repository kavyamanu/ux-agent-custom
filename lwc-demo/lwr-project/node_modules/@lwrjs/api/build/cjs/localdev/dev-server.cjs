...
async function createDevServer(port, bundleProdDir) {
  ...
  const configPath = (0, import_shared_utils.crossEnvFileURL)(import_path.default.join(process.cwd(), "app", "ssr.js"));
  const {
  ...
  if (bundleProdDir) {
    hooks = [import_path.default.join(__dirname, "hooks", "prod-context-hook.js")];
    bundleProviders = [
      [
        import_path.default.join(__dirname, "providers", "override-bundle-provider.js"),
  ...
  let lwcCompiler;
  const lwcCompilerPath = import_path.default.join(process.cwd(), "app", "localdev", "lwc-compiler.js");
