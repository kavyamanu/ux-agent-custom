...
var StaticOverrideBundleProvider = class extends import_bundle_provider.default {
  ...
  async bundle(moduleId, runtimeEnvironment, runtimeParams) {
    ...
    const metadata = super.getBundleMetadata(moduleId, "en-US", !this.writeProdBundlesDir);
    const fullPath = metadata?.path || "";
    const dirname = import_path.default.basename(import_path.default.dirname(fullPath));
    const fileName = moduleId.specifier.split("@view/")[1] + "_view.js";
    if (this.writeProdBundlesDir) {
      const prodPath = import_path.default.join(this.writeProdBundlesDir, "site", "prod", "view", "en-US", dirname, fileName);
    ...
    const cachePath = (0, import_shared_utils.getLocalDevOverrideUrl)(this.cacheDir, moduleId.specifier, fullPath);
