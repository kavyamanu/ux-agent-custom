...
const visitors = {
    ...
    ClassDeclaration(path, state) {
        ...
        if (path.node.superClass.type === 'Identifier' &&
            ...
            if (path.node.id) {
                ...
                path.node.id = builders.identifier('DefaultComponentName');
    ...
    MethodDefinition(path, state) {
        ...
        switch (path.node.key.name) {
            case 'constructor':
                path.node.value.params = [builders.identifier('propsAvailableAtConstruction')];
                break;
    ...
    Super(path, _state) {
        ...
        if (parentFn &&
            ...
            path.parentPath.node.arguments = [builders.identifier('propsAvailableAtConstruction')];
        }
    },
};
...
function getChildAttrsOrProps(attrs, cxt) {
    const objectAttrsOrProps = attrs.map((attr) => {
        const key = isValidIdentifier(attr.name) ? builders.identifier(attr.name) : builders.literal(attr.name);
        if (attr.value.type === 'Literal' && typeof attr.value.value === 'string') {
            const value = attr.name === 'style' ? cleanStyleAttrVal(attr.value.value) : attr.value.value;
            return builders.property('init', key, builders.literal(value));
        }
        else if (attr.value.type === 'Literal' && typeof attr.value.value === 'boolean') {
            return builders.property('init', key, builders.literal(attr.value.value));
        }
...
function yieldAttrOrPropLiteralValue(name, valueNode) {
    ...
    if (typeof value === 'string') {
        ...
        return [bYield$1(builders.literal(` ${name}`))];
    }
...
const Text = function Text(node, cxt) {
    if (isLiteral(node.value)) {
        return [bYield(builders.literal(node.value.value))];
    }
    const isIsolatedTextNode = builders.literal((!cxt.prevSibling || cxt.prevSibling.type !== 'Text') &&
