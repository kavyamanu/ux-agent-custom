...
async function getLLMResponse(availableComponents, userPrompt) {
    // Extract unique page names from components
    const availablePageNames = [
        ...new Set(availableComponents.map((comp) => comp.containing_frame.pageName)),
    ];
    try {
...
async function renderComponents(userPrompt) {
    try {
        ...
        const width = designData.layout && designData.layout.width
            ? designData.layout.width
            : 400;
        const height = designData.layout && designData.layout.height
            ? designData.layout.height
            : 300;
        const x = designData.layout && designData.layout.x ? designData.layout.x : 0;
        const y = designData.layout && designData.layout.y ? designData.layout.y : 0;
        ...
        for (const child of designData.children) {
            try {
                ...
                const matchingComponent = components.find((comp) => comp.containing_frame.pageName.toLowerCase() ===
                    child.type.toLowerCase());
                if (matchingComponent) {
                    console.log(`Found matching component for ${child.type}:`, matchingComponent);
                    ...
                    instance.resize(child.layout ? child.layout.width : 100, child.layout ? child.layout.height : 40);
                    ...
                    figma.notify(`No matching component found for ${child.componentName}`, { error: true });
                }
            }
            catch (error) {
                console.error(`Error rendering component ${child.componentName}:`, error);
